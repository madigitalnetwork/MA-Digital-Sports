...
/* Route a Cricbuzz image URL through our own /api/img proxy instead of
   pointing the browser straight at their CDN — sidesteps any Referer/hotlink
   check on their end, since the actual cross-origin fetch happens server-side
   from our function, not the viewer's browser. */
...
function logoUrl(t){
  ...
  if (id === null) return "";
  return proxied(`https://static.cricbuzz.com/a/img/v1/300x300/i1/c${id}/${slugName(t.teamName || t.name)}.jpg`);
}
...
function toRow(node, forcedMs){
  ...
  return {
    ...
    team1Id:     num(info.team1 && (info.team1.teamId ?? info.team1.teamid)),
    team2Id:     num(info.team2 && (info.team2.teamId ?? info.team2.teamid)),
...
function playerLogoUrl(o){
  const id = pId(o);
  if (id === null) return "";
  return proxied(`https://static.cricbuzz.com/a/img/v1/300x300/i1/c${id}/${slugName(pName(o))}.jpg`);
}
...
function parseScoreboard(resp){
  ...
  // Pick the CURRENT innings' overs — a Test lists completed innings too, so
  // the last entry can be a finished innings with far more overs. Match the
  // innings whose runs equal the live batting score; fall back to the batting
  // team's latest innings, then the last entry.
  const inns = (mini.inningsscores && mini.inningsscores.inningsscore) || [];
  let inn = null;
  if (Array.isArray(inns) && inns.length){
    const teamId = sc && (sc.teamid ?? sc.teamId);
    const byScore = runs !== null ? inns.find(x => x && num(x.runs) === runs) : null;
    const byTeam  = teamId != null ? inns.filter(x => x && (x.batteamid ?? x.batTeamId) === teamId) : [];
    inn = byScore || (byTeam.length ? byTeam[byTeam.length - 1] : null) || inns[inns.length - 1];
  }
...
/* ------------------------------------------------------------------ *
 *  Squad photos — the leanback never carries a real faceImageId, but the
 *  per-team squad endpoint does. Fetch each team's squad once and cache it
 *  in memory (keyed by matchId) for a few hours — a squad does not change
 *  during a match, so this costs at most two extra calls for the whole game,
 *  not one per poll. Serverless instances are short-lived, so this is a
 *  best-effort cache (a cold start just re-fetches), never a correctness
 *  requirement.
 */
const SQUAD_CACHE_MS = 6 * 60 * 60 * 1000;
const squadCache = new Map();   // matchId -> { at, byName: Map(normalisedName -> photoUrl) }

function normName(s){ return (s || "").toString().toLowerCase().trim().replace(/\s+/g, " "); }

/* 182026 is Cricbuzz's own "no photo available" placeholder id — treat it as
   no photo so we fall back to our own blank silhouette instead of a canned
   generic image that would be identical across many different players. */
const NO_FACE_ID = 182026;

async function fetchTeamSquad(key, matchId, teamId){
  const resp = await apiGet(`/mcenter/v1/${encodeURIComponent(matchId)}/team/${encodeURIComponent(teamId)}`, key);
  const groups = (resp && resp.player) || [];
  const out = [];
  groups.forEach(g => (g.player || []).forEach(p => out.push(p)));
  return out;
}

/* Returns { byName, captainPhotos:{ team1, team2 } }. captainPhotos lets the
   caller show each side's actual captain in the captain box instead of the
   team crest, keyed by "team1"/"team2" (matching the caller's own team1Id/
   team2Id order) since the caller — not this function — knows which of
   those is currently batting vs bowling. */
async function fetchSquadMap(key, matchId, team1Id, team2Id){
  const cached = squadCache.get(matchId);
  if (cached && Date.now() - cached.at < SQUAD_CACHE_MS){
    return { byName: cached.byName, captainPhotos: cached.captainPhotos };
  }

  const byName = new Map();
  const captainPhotos = { team1:"", team2:"" };
  const slots = [["team1", team1Id], ["team2", team2Id]].filter(([, id]) => id != null);
  const lists = await Promise.allSettled(slots.map(([, id]) => fetchTeamSquad(key, matchId, id)));
  ...
  lists.forEach((r, i) => {
    ...
    const [slot] = slots[i];
    r.value.forEach(p => {
      const faceId  = num(p.faceimageid ?? p.faceImageId);
      const hasPhoto = !!(p.name && faceId && faceId !== NO_FACE_ID);
      if (!hasPhoto) return;
      const url = proxied(`https://static.cricbuzz.com/a/img/v1/300x300/i1/c${faceId}/${slugName(p.name)}.jpg`);
      byName.set(normName(p.name), url);
      if (p.captain === true || p.iscaptain === true) captainPhotos[slot] = url;
    });
  });

  // Only cache a genuinely successful fetch. If BOTH team-squad calls failed
  // (a transient hiccup, rate limit, etc.) leave the cache empty so the next
  // poll tries again immediately instead of being stuck with no photos for
  // the full cache window.
  if (anyFulfilled) squadCache.set(matchId, { at: Date.now(), byName, captainPhotos });
  return { byName, captainPhotos };
}

/* Fill in any batter/bowler photo that the leanback left blank, using the
   squad's real face photo when the name matches. Best-effort: if the squad
   fetch fails, the scoreboard still renders with placeholders. */
function applySquadPhotos(state, byName){
  if (!byName || !byName.size) return;
  (state.batters || []).forEach(b => {
    if (!b.photo){ const url = byName.get(normName(b.name)); if (url) b.photo = url; }
  });
  if (state.bowler && !state.bowler.photo){
    const url = byName.get(normName(state.bowler.name));
    if (url) state.bowler.photo = url;
  }
}

async function fetchScoreboardState(key, matchId, team1Id, team2Id){
  if (!matchId) return {};
  ...
  const state = parseScoreboard(resp);

  if (team1Id != null || team2Id != null){
    try {
      const squad = await fetchSquadMap(key, matchId, team1Id, team2Id);
      applySquadPhotos(state, squad.byName);
      state.captainPhotos = squad.captainPhotos;   // { team1, team2 } — caller maps to teamA/teamB
    } catch (_) { /* photos are optional */ }
  }
...
/* ------------------------------------------------------------------ *
 *  Full scorecard — every bowler's figures, fall of wickets, extras.
 *  A heavier endpoint than leanback (the full innings breakdown, not just
 *  the two batters + bowler at the crease), so it is only fetched when the
 *  operator opens the scorecard panel, not on every 4s poll. Same
 *  defensive-parsing approach as the rest of this file: try several known
 *  field-name variants and degrade to empty rather than throw, since this
 *  endpoint's exact shape hasn't been exercised against live data yet.
 * ------------------------------------------------------------------ */
function parseFullScorecard(resp){
  const out = { teamName:"", bowlers:[], fow:[], extras:0 };
  if (!resp || typeof resp !== "object") return out;

  // One entry per innings; the last one is the innings in progress (or the
  // most recently completed one, for a finished match).
  const cards = deepFindArray(resp, ["scoreCard", "scorecard", "scoreCards"]);
  const card  = cards && cards.length ? cards[cards.length - 1] : resp;
  if (!card || typeof card !== "object") return out;

  const bowlTeam   = deepFindObject(card, ["bowlTeamDetails", "bowlingTeamDetails", "bowlTeam"]) || card;
  const bowlersRaw = bowlTeam && (bowlTeam.bowlersData || bowlTeam.bowlers || bowlTeam.bowlerData);
  const bowlersArr = Array.isArray(bowlersRaw) ? bowlersRaw
    : (bowlersRaw && typeof bowlersRaw === "object" ? Object.values(bowlersRaw) : null);

  if (bowlersArr){
    out.bowlers = bowlersArr.map(b => {
      if (!b || typeof b !== "object") return null;
      const name = pName(b) || b.bowlName || b.bowlerName || "";
      if (!name) return null;
      const overs = num(b.bowlOvers ?? b.overs ?? b.o) || 0;
      const runs  = num(b.bowlRuns  ?? b.runs  ?? b.r) || 0;
      return {
        name,
        overs,
        maidens: num(b.bowlMaidens ?? b.maidens ?? b.m) || 0,
        runs,
        wickets: num(b.bowlWkts ?? b.wickets ?? b.wkts ?? b.w) || 0,
        econ:    num(b.bowlEcon ?? b.economy ?? b.econ) || (overs ? +(runs / overs).toFixed(2) : 0)
      };
    }).filter(Boolean);
  }

  const batTeam = deepFindObject(card, ["batTeamDetails", "battingTeamDetails", "batTeam"]) || card;
  out.teamName = (batTeam && (batTeam.batTeamName || batTeam.teamName || batTeam.name)) || "";

  const wktsRaw = deepFindObject(card, ["wicketsData", "fallOfWickets", "fow"]) ||
                  deepFindArray(card, ["wicketsData", "fallOfWickets", "fow"]);
  if (wktsRaw){
    const arr = Array.isArray(wktsRaw) ? wktsRaw : Object.values(wktsRaw);
    out.fow = arr
      .map(w => (w && typeof w === "object") ? {
        score: num(w.wktRunsScored ?? w.runs ?? w.score) || 0,
        num:   num(w.wktNum ?? w.wicketNumber ?? w.num) || 0
      } : null)
      .filter(w => w && w.num)
      .sort((a, b) => a.num - b.num);
  }

  const extrasObj = deepFindObject(card, ["extrasData", "extras"]);
  out.extras = num(extrasObj && (extrasObj.total ?? extrasObj.extrasTotal ?? extrasObj.r)) || 0;

  return out;
}

async function fetchFullScorecard(key, matchId){
  if (!matchId) return { teamName:"", bowlers:[], fow:[], extras:0 };
  const resp = await apiGet(`/mcenter/v1/${encodeURIComponent(matchId)}/scard`, key);
  return parseFullScorecard(resp);
}
...
/* The scoreboard's hot path. Brings in live + recent + schedule together —
   a multi-day match (e.g. a Test) drops off the "live" feed during a session
   break, lunch, tea or overnight, and only "recent" still lists it with its
   last known score, so the pinned match must not go missing just because
   nobody is bowling this exact second. Concatenated live-first so dedupe
   prefers the freshest state when a match briefly appears in more than one
   feed during a state transition. Only the live feed is treated as required;
   recent/schedule failures are tolerated. */
async function fetchLiveRows(key, pickId){
  const [liveR, recentR, schedR] = await Promise.allSettled([
    fetchLive(key), fetchRecent(key), fetchSchedule(key)
  ]);
  const rows = (liveR.status === "fulfilled" ? liveR.value : [])
    .concat(recentR.status === "fulfilled" ? recentR.value : [])
    .concat(schedR.status === "fulfilled" ? schedR.value : []);
  if (liveR.status === "rejected") throw liveR.reason;   // the live feed is not optional
  return dedupe(rows);
}
...
function normalise(rows, pickId){
  ...
  return {
    ...
    teamA:{
      ...
      captain:"Batting", photo:battingLogo || `assets/img/${slug(battingT.short)}-captain.png`
    },
    teamB:{
      name:bowlingT.name, short:bowlingT.short,
      flag:bowlingLogo || `assets/img/${slug(bowlingT.short)}-flag.jpg`,
      captain:"Bowling", photo:bowlingLogo || `assets/img/${slug(bowlingT.short)}-captain.png`
    },
    runs:       batting.runs,
...
module.exports = {
  ...
  fetchFullScorecard, parseFullScorecard,
