...
async function fetchSquadMap(key, matchId, team1Id, team2Id){
  ...
  if (cached && Date.now() - cached.at < SQUAD_CACHE_MS) return cached.byName;

  const byName = new Map();
  const ids = [team1Id, team2Id].filter(id => id != null);
  const lists = await Promise.allSettled(ids.map(id => fetchTeamSquad(key, matchId, id)));
  ...
  lists.forEach(r => {
    ...
    r.value.forEach(p => {
      ...
      if (!p.name || !faceId || faceId === NO_FACE_ID) return;
      byName.set(normName(p.name), proxied(`https://static.cricbuzz.com/a/img/v1/300x300/i1/c${faceId}/${slugName(p.name)}.jpg`));
    });
  });
  ...
  if (anyFulfilled) squadCache.set(matchId, { at: Date.now(), byName });
  return byName;
}
...
async function fetchScoreboardState(key, matchId, team1Id, team2Id){
  ...
  if (team1Id != null || team2Id != null){
    try {
      const byName = await fetchSquadMap(key, matchId, team1Id, team2Id);
      applySquadPhotos(state, byName);
