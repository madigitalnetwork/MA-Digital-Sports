...
function parseScoreboard(resp){
  ...
  if (ov !== null && ov !== undefined){
    let o = Math.floor(ov);
    let b = Math.round((ov - o) * 10);
    // Cricbuzz's own overs field can briefly read e.g. 95.6 right as an over
    // completes — cricket notation never goes past .5 (6 legal balls), so
    // roll that into the next over rather than showing an invalid ".6".
    if (b >= 6){ o += 1; b = 0; }
    out.overs = o;
    out.ballInOver = b;
  }
 ...
 *  operator opens the scorecard panel, not on every 4s poll.
 *
 *  Confirmed shape (from a live Test match response):
 *    { scorecard: [ { inningsid, batsman:[...], bowler:[ {name, nickname,
 *        overs, maidens, runs, wickets, economy}, ... ],
 *        fow:{ fow:[ {batsmanname, overnbr, runs, ballnbr}, ... ] },
 *        extras:{ legbyes, byes, wides, noballs, penalty, total },
 *        batteamname, score, wickets, overs, ... }, ... ],
 *      status, ismatchcomplete }
 *  scorecard[] has one entry per innings; the last entry is the innings in
 *  progress, and its `bowler` array is the CURRENT bowling side's figures —
 *  exactly the side normalise() calls teamB. Fall-of-wicket entries don't
 *  carry their own wicket number, only arrival order, so that's the index.
 * ------------------------------------------------------------------ */
function parseFullScorecard(resp){
  const out = { teamName:"", bowlers:[], fow:[], extras:0 };
  if (!resp || typeof resp !== "object") return out;

  const cards = Array.isArray(resp.scorecard) ? resp.scorecard : null;
  const card  = cards && cards.length ? cards[cards.length - 1] : null;
  if (!card) return out;

  out.teamName = card.batteamname || card.batteamsname || "";

  if (Array.isArray(card.bowler)){
    out.bowlers = card.bowler.map(b => {
      if (!b || typeof b !== "object") return null;
      const name = b.nickname || b.name || "";
      if (!name) return null;
      return {
        name,
        overs:   b.overs != null ? String(b.overs) : "0",
        maidens: num(b.maidens) || 0,
        runs:    num(b.runs) || 0,
        wickets: num(b.wickets) || 0,
        econ:    parseFloat(b.economy) || 0
      };
    }).filter(Boolean);
  }

  const fowArr = card.fow && Array.isArray(card.fow.fow) ? card.fow.fow : null;
  if (fowArr){
    out.fow = fowArr.map((w, i) => ({ num: i + 1, score: num(w && w.runs) || 0 }));
  }

  out.extras = num(card.extras && card.extras.total) || 0;
...
async function fetchFullScorecard(key, matchId){
  ...
  const resp = await apiGet(`/mcenter/v1/${encodeURIComponent(matchId)}/hscard`, key);
...
function parseScore(text){
  ...
  let overs = Math.floor(ov);
  let ball  = Math.round((ov - overs) * 10);
  // Same over-notation rollover as parseScoreboard: never show a 6th ball
  // of an over — that over is actually complete.
  if (ball >= 6){ overs += 1; ball = 0; }
  return { runs:+m[1], wickets:+m[2], overs, ball };
}
