...
/* The scoreboard's hot path. Always brings in the schedule alongside the live
   feed — the "upcoming" marquee needs it even when the pinned match is
   already live — but tolerates the schedule call failing or being empty. */
async function fetchLiveRows(key, pickId){
  const [liveR, schedR] = await Promise.allSettled([fetchLive(key), fetchSchedule(key)]);
