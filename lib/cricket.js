...
function logoUrl(t){
  ...
  if (id === null) return "";
  return `https://static.cricbuzz.com/a/img/v1/300x300/i1/c${id}/${slugName(t.teamName || t.name)}.jpg`;
}
...
function playerLogoUrl(o){
  const id = pId(o);
  if (id === null) return "";
  return `https://static.cricbuzz.com/a/img/v1/300x300/i1/c${id}/${slugName(pName(o))}.jpg`;
}
...
async function fetchSquadMap(key, matchId, team1Id, team2Id){
  ...
  lists.forEach(r => {
    ...
    r.value.forEach(p => {
      ...
      byName.set(normName(p.name), `https://static.cricbuzz.com/a/img/v1/300x300/i1/c${faceId}/${slugName(p.name)}.jpg`);
    });
  });
