...
async function fetchLiveRows(key, pickId){
  ...
  const found   = pickId && rows.some(r => r.id === String(pickId) && r.ms === "live");
  const hasLive = rows.some(r => r.ms === "live");
  // Only spend an extra upstream call on the schedule when we actually need it:
  // the pinned match isn't live yet, or nothing is live at all.
  if (!found && (pickId || !hasLive)){
